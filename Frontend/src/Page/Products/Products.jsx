...
const Products = () => {
    ...
    const getData = async () => {
        let api = await axios.get("http://127.0.0.1:8000/product");
        setData(api.data);
    };
    const getCat=async()=>{
        let cat=await axios.get("http://127.0.0.1:8000/category")
        setCat(cat.data)
    }
    useEffect(() => {
        getData();
    ...
    return (
        <>
            <NavbarMain home={false} product={true} company={false} event={false} cari={(e) => setSearchTerm(e.target.value)}/>
                ...
                <div className="flex flex-wrap gap-3 mt-3">
                    {
                        ...
                        cat.map((items)=>{
                            return(
                                <Button label={items.category} className="bg-blue-950 border-blue-950"/>
                            )
                        })
                    }
                </div>
